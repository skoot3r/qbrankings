...
const TEAM_COLORS={BUF:['#00338D','#C60C30'],KC:['#E31837','#FFB81C'],BAL:['#241773','#9E7C0C'],LAR:['#003594','#FFA300'],CIN:['#FB4F14','#000000'],DAL:['#003594','#869397'],DET:['#0076B6','#B0B7BC'],LAC:['#0080C6','#FFC20E'],GB:['#203731','#FFB612'],CHI:['#0B162A','#C83803'],NE:['#002244','#C60C30'],JAX:['#006778','#D7A22A'],WAS:['#5A1414','#FFB612'],PHI:['#004C54','#A5ACAF'],DEN:['#FB4F14','#002244'],TB:['#D50A0A','#FF7900'],SEA:['#002244','#69BE28'],SF:['#AA0000','#B3995D'],HOU:['#03202F','#A71930'],NO:['#101820','#D3BC8D'],ARI:['#97233F','#000000'],CAR:['#0085CA','#101820'],NYJ:['#125740','#FFFFFF'],PIT:['#FFB612','#101820'],ATL:['#A71930','#000000'],CLE:['#311D00','#FF3C00'],MIA:['#008E97','#FC4C02'],NYG:['#0B2265','#A71930'],TEN:['#0C2340','#4B92DB'],MIN:['#4F2683','#FFC62F'],IND:['#002C5F','#A2AAAD'],LV:['#000000','#A5ACAF']};
function teamClass(team){return 'team-'+String(team||'').toUpperCase().replace(/[^A-Z0-9]/g,'');}
function teamStyle(team){const c=TEAM_COLORS[String(team||'').toUpperCase()];return c?` style="--team-primary:${c[0]};--team-secondary:${c[1]}"`:'';}
const TEAM_LOGO_CODES={WAS:'wsh'};
function teamLogoUrl(team){const t=String(team||'').toUpperCase();const code=TEAM_LOGO_CODES[t]||t.toLowerCase();return `https://a.espncdn.com/i/teamlogos/nfl/500/${code}.png`;}
function localSeed(){
...
window.renderHome=async function(){
  ...
  function draw(){
    ...
    cards.innerHTML=d.players.map((p,i)=>{const old=prev?prev.players.findIndex(x=>(x.id&&p.id&&x.id===p.id)||x.name===p.name):-1;const m=old<0?0:old-i,cls=m>0?'up':m<0?'down':'same',label=m>0?'▲ '+m:m<0?'▼ '+Math.abs(m):'—';return {...p,i,m,cls,label};}).filter(x=>(!q||x.name.toLowerCase().includes(q))&&(f==='all'||(f==='up'&&x.m>0)||(f==='down'&&x.m<0)||(f==='same'&&!x.m))).map(x=>`<article class="card ${teamClass(x.team)}"${teamStyle(x.team)} tabindex="0" role="button" aria-expanded="false"><div class="rank">${x.i+1}</div><div class="movement ${x.cls}" aria-label="Rank change">${x.label}</div><div class="player-identity">${x.headshot?`<img class="qb-headshot" src="${esc(x.headshot)}" alt="" loading="lazy" onerror="this.parentElement.classList.add('headshot-missing');this.remove()">`:''}<img class="team-logo" src="${teamLogoUrl(x.team)}" alt="" loading="lazy" onerror="this.remove()"><div class="player-copy"><div class="name-line"><div class="name">${esc(x.name)}</div><div class="team">${esc(x.team)} <span class="position-tag">QB</span></div></div></div></div><div class="qb-stats card-stats"><span><b>${esc(x.stats?.passYards??0)}</b><small>YDS</small></span><span><b>${esc(x.stats?.td??0)}</b><small>TD</small></span><span><b>${esc(x.stats?.int??0)}</b><small>INT</small></span><span><b>${esc(Number(x.stats?.compPct??0).toFixed(1))}%</b><small>CMP</small></span></div><div class="note-cell"><div class="note-label">Notes <span class="note-chevron">⌄</span></div><div class="note-text">${esc(x.note||'')}</div></div></article>`).join('');
...
window.renderHistory=async function(){
  ...
  function showWeek(week){
    ...
    const cards=players.map((p,i)=>{const old=prev?prev.players.findIndex(x=>(x.id&&p.id&&x.id===p.id)||x.name===p.name):-1;const m=old<0?0:old-i;const cls=m>0?'up':m<0?'down':'same';const label=m>0?'▲ '+m:m<0?'▼ '+Math.abs(m):'—';return {...p,i,m,cls,label};}).map(x=>`<article class="card ${teamClass(x.team)}"${teamStyle(x.team)} tabindex="0" role="button" aria-expanded="false"><div class="rank">${x.i+1}</div><div class="movement ${x.cls}" aria-label="Rank change">${x.label}</div><div class="player-identity">${x.headshot?`<img class="qb-headshot" src="${esc(x.headshot)}" alt="" loading="lazy" onerror="this.parentElement.classList.add('headshot-missing');this.remove()">`:''}<img class="team-logo" src="${teamLogoUrl(x.team)}" alt="" loading="lazy" onerror="this.remove()"><div class="player-copy"><div class="name-line"><div class="name">${esc(x.name)}</div></div><div class="team">${esc(x.team)} <span class="position-tag">QB</span></div></div></div><div class="qb-stats card-stats"><span><b>${esc(x.stats?.passYards??0)}</b><small>YDS</small></span><span><b>${esc(x.stats?.td??0)}</b><small>TD</small></span><span><b>${esc(x.stats?.int??0)}</b><small>INT</small></span><span><b>${esc(Number(x.stats?.compPct??0).toFixed(1))}%</b><small>CMP</small></span></div><div class="note-cell"><div class="note-label">Notes <span class="note-chevron">⌄</span></div><div class="note-text">${esc(x.note||'')}</div></div></article>`).join('');
